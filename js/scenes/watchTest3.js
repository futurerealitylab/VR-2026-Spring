...
export const init = async model => {
   ...
   //I tried sending over longer functions ahead of time, startVib and endVib, in this version.
   //These are called below. My thinking was it is cleaner and also easier to send a few bytes just calling the function to the watch
   //This doesn't work well though. It does work, but sometimes the watch freaks out when trying to clear the ram and restart the program,
   //and then the vibration motor stops working until the watch is rebooted. I am not sure why, but I suspect the functions are
   //somehow not clearing fully.
   watchCommand = 
               "function makeWatchBuzz() {Bangle.buzz(500,0.2);}\n" + 
               "function startVib() {\n" + "g.clear(1);\n" +
                           "g.setFontAlign(0,0);\n" +
                           "g.setFont('Vector', 20);\n" +
